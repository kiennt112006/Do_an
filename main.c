...
int main(void)
{
  ...
  // MPU6050 mặc định ở chế độ Sleep. Gửi 0x00 vào thanh ghi 0x6B để đánh thức
    ...
    /* USER CODE BEGIN 2 */
      // MPU6050 mặc định ở chế độ Sleep. Gửi 0x00 vào thanh ghi 0x6B để đánh thức
      data = 0; // Tái sử dụng biến data đã khai báo phía trên, không khai báo lại uint8_t
      HAL_I2C_Mem_Write(&hi2c1, MPU6050_ADDR, 0x6B, 1, &data, 1, 1000);
      HAL_Delay(50); // Chờ cảm biến ổn định

      // === CODE CHẠY ĐỒNG THỜI CẢ 2 ĐỘNG CƠ ===

      // 1. Kích hoạt bộ tạo xung PWM cho cả Kênh 1 (Bánh trái) và Kênh 2 (Bánh phải)
      ...
      // 4. Cấp tốc độ băm xung PWM cho cả 2 bên (Dải giá trị từ 0 đến 100)
      __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 30); // Bánh trái chạy 30% tốc độ
      __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_2, 30); // Bánh phải chạy 30% tốc độ
  ...
  /* Infinite loop */
    ...
    while (1)
    {
      ...
      /* USER CODE BEGIN 3 */
  	  ...
  	  // Xong! Không cần UART hay Bluetooth gì ở đây cả.
        // Chỉ cần Delay để giữ đúng nhịp 100Hz (10ms)
  	  HAL_Delay(10);
    }
    /* USER CODE END 3 */
}
